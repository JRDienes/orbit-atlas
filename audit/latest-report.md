...
|---|---|---|---|
| First Contentful Paint (FCP) | 0.3 s | Good ✅ |  ↓108ms faster |
| Largest Contentful Paint (LCP) | 0.7 s | Good ✅ |  ↓366ms faster |
| Total Blocking Time (TBT) | 4,870 ms | Poor ❌ |  ↑754ms slower |
| Cumulative Layout Shift (CLS) | 0.003 | Good ✅ |  → |
| Speed Index | 9.4 s | Poor ❌ | |
| Time to Interactive (TTI) | 28.1 s | Poor ❌ |  ↓610ms faster |
...
Total Blocking Time is 4,870 ms. Break large `postMessage` payloads from the satellite worker into smaller chunks so the main thread is never blocked > 50 ms.
