...
| Largest Contentful Paint (LCP) | 1.5 s | Needs Improvement ⚠️ |  ↓162ms faster |
| Total Blocking Time (TBT) | 4,040 ms | Poor ❌ |  ↑2832ms slower |
| Cumulative Layout Shift (CLS) | 0.003 | Good ✅ |  → |
| Speed Index | 9.3 s | Poor ❌ | |
| Time to Interactive (TTI) | 27.9 s | Poor ❌ |  ↓485ms faster |
...
Total Blocking Time is 4,040 ms. Break large `postMessage` payloads from the satellite worker into smaller chunks so the main thread is never blocked > 50 ms.
