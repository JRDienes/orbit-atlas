...
|---|---|---|---|
| First Contentful Paint (FCP) | 0.3 s | Good ✅ |  ↓95ms faster |
| Largest Contentful Paint (LCP) | 0.9 s | Good ✅ |  ↑115ms slower |
| Total Blocking Time (TBT) | 2,970 ms | Poor ❌ |  ↓1057ms faster |
| Cumulative Layout Shift (CLS) | 0.002 | Good ✅ |  → |
| Speed Index | 9.0 s | Poor ❌ | |
| Time to Interactive (TTI) | 27.9 s | Poor ❌ |  ↓1213ms faster |
...
Total Blocking Time is 2,970 ms. Break large `postMessage` payloads from the satellite worker into smaller chunks so the main thread is never blocked > 50 ms.

**Files:** `orbit-atlas-web/src/satWorker.js`

### 2. [HIGH] TTI

Time to Interactive is 27.9 s. Defer satellite worker initialization until after first paint; show the loading overlay immediately to unblock the main thread.
