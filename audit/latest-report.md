...
| Largest Contentful Paint (LCP) | 0.8 s | Good ✅ |  ↓96ms faster |
| Total Blocking Time (TBT) | 4,650 ms | Poor ❌ |  → |
| Cumulative Layout Shift (CLS) | 0.003 | Good ✅ |  → |
| Speed Index | 9.6 s | Poor ❌ | |
| Time to Interactive (TTI) | 28.2 s | Poor ❌ |  ↓309ms faster |
...
Total Blocking Time is 4,650 ms. Break large `postMessage` payloads from the satellite worker into smaller chunks so the main thread is never blocked > 50 ms.
